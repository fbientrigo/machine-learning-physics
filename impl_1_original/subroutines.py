...
def return_HM_npy(csvfile_path):
    """
    simple reader of csv data to Hs, Ms
    where Hs and Ms is:
    """
    Hs = []
    Ms = []
...
def generate_training_data(noise_std=0.004, data_size=10000, Hrange=[0, 0.022], Mrange=[0, 0.11], f=None):
    ...
    """
    # positives and negatives are with respect to a function
    # H, M, F positives
    H_pos_data = []
    M_pos_data = []
    F_pos_data = []
    ...
    #
    while True:
        # assuming uniform distribution inside the range H,M: [0, 0.022], [0, 0.11]
        ...
        if len(F_pos_data)==data_size and len(F_neg_data)==data_size:
            break
    # returns 2 tuples (H_pos, M_pos, F_pos), (H_neg, M_neg, F_neg)
...
def plot_all(Hs, Ms, positive_H_M_F, negative_H_M_F, f):
    """
    Having data in a 2 class system, close enough to `f` and far away, they are plotted with different colores
    also plotting the `f` fitted function.

    H_M_F is shorthand for a touple of arrays (Hs, Ms, Fs)
    Hs and Ms being array of floats
    Fs being array of 1 and 0's
    """
